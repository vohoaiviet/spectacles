...
public class FCTHReranker {
	IndexReader mReader;
	
	public FCTHReranker(String indexDirectory) throws CorruptIndexException, IOException {
	...
	public List<Score> filterResults(BufferedImage image, String filename,List<Score> originalScores) throws IOException{
		...
		for ( Score score : originalScores ){
			...
			FCTH db = new FCTH();
			db.setStringRepresentation(doc.get(DocumentBuilder.FIELD_NAME_FCTH));
	...
	public static void main(String args[]){
		try {
			FCTHReranker reranker = new FCTHReranker("D:\\small\\web-index\\fcth");
