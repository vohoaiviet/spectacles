...
public class RandomReranker {
	IndexReader mReader;
	
	public RandomReranker(String indexDirectory) throws CorruptIndexException, IOException {
	...
	public List<Score> filterResults(BufferedImage image, String filename,List<Score> originalScores) throws IOException{
		ColorLayout desc = new ColorLayout();
		desc.extract(image);
		for ( Score score : originalScores ){
			...
			ColorLayout db = new ColorLayout();
			db.setStringRepresentation(doc.get(DocumentBuilder.FIELD_NAME_COLORLAYOUT));
	...
	public static void main(String args[]){
		try {
			RandomReranker reranker = new RandomReranker("D:\\small\\web-index\\rand");
